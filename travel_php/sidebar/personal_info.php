                        ...
                        if($g_prof_size < 3000000){
                            ...
                            $g_update=$con->prepare("UPDATE `addis_abeba_tour`.`guides` set 
                                            g_fname=?, g_lname=?, g_email=?, g_nationality=?,g_city=?,g_subcity=?, g_phone_num=?, g_qualification=?, g_languages=?,g_profile_image=?
                                                    where g_id=?");
                            $confirm=$g_update->execute([$fname,$lname,$email,$nationality,$city,$subcity,$phone_num,$qualification,$language,$img_new_name,$session_id]);
...
?>
        ...
        <div class="contents">
            ...
            <div class="datas">
                ...
                <P><?php if(isset($_COOKIE['gcookie'])){
        $session_id=$_COOKIE['gcookie'];
    } ?>
    </P>
